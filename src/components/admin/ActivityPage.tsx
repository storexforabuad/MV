...
const ActivityPage: React.FC<ActivityPageProps> = ({
  ...
  // Growth Tips Logic
  const growthTips = [
    "Share your top 3 products on WhatsApp Status right now to hit your daily goal!",
    "Running a discount? Broadcast it to your customers using the Social Hub.",
    "Did you know? Products with more than 50 views are 3x more likely to sell.",
    "Add more categories to help customers find exactly what they want faster.",
    "Check your low stock items and update them before you miss a sale!"
  ];

  const dailyTip = growthTips[new Date().getDate() % growthTips.length];
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[2rem] p-6 shadow-sm relative overflow-hidden">
        ...
        <p className="text-zinc-800 dark:text-zinc-200 font-bold text-lg leading-tight">
          &quot;{dailyTip}&quot;
        </p>
        <button
          onClick={openSocialModal}
          className="mt-4 flex items-center gap-2 text-blue-600 dark:text-blue-400 font-black text-sm uppercase tracking-tighter hover:gap-3 transition-all"
        >
          Take Action <ArrowRight className="h-4 w-4" strokeWidth={3} />
        </button>
      </motion.div>
