    ...
    return (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
            ...
            <motion.div
                ...
                className="relative w-full sm:max-w-2xl bg-black border border-slate-800 sm:rounded-[40px] rounded-t-[40px] max-h-[92vh] flex flex-col overflow-hidden text-white font-sans shadow-2xl"
            >
                {/* Header */}
                <header className="px-6 py-6 pb-4 flex-shrink-0 border-b border-slate-800/50 relative overflow-hidden">
                    ...
                    <div className="relative z-10 flex items-start justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-400 font-medium">Referral Network Dashboard</p>
                            </div>
                        </div>
                        <button onClick={onClose} className="w-10 h-10 bg-slate-900 rounded-full flex items-center justify-center text-slate-400 hover:text-white transition-colors">
                            <X className="w-5 h-5" />
                        </button>
                    </div>
                </header>
                        ...
                        <div className="pb-10 pt-4">
                            ...
                            <div className="px-6 mb-6">
                                <motion.div
                                    ...
                                    className="bg-slate-900/40 border border-slate-800 rounded-3xl p-5 backdrop-blur-sm cursor-pointer hover:border-emerald-500/30 transition-colors group relative overflow-hidden"
                                >
                                    ...
                                    <div className="relative z-10">
                                        ...
                                        <div className="flex items-center justify-between text-[10px] font-bold">
                                            <span className={`px-2 py-0.5 rounded-full border border-emerald-500/20 bg-emerald-500/10 text-emerald-400 uppercase`}>
                                                {data.tier.name} Tier
                                            </span>
                                            {data.tier.nextTierThreshold && (
                                                <span className="text-slate-500">
                                                    Add <span className="text-white font-black">{data.tier.nextTierThreshold - data.summary.activeStores}</span> more active stores to level up
                                                </span>
                                            )}
                                        </div>
                                    </div>
                                ...
                                <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-3xl p-5">
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            ...
                                            className="flex-1 bg-slate-900 border border-slate-800 py-3 rounded-2xl text-xs font-bold flex items-center justify-center gap-2 active:scale-95 transition-all text-white hover:bg-slate-800"
                                        >
                            ...
                            <div className="px-6 mb-4">
                                <div className="flex bg-slate-900 p-1 rounded-2xl border border-slate-800 select-none">
                                    <button
                                        onClick={() => setActiveTab('stores')}
                                        className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'stores' ? 'bg-slate-800 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}
                                    >
                                        Stores
                                    </button>
                                    <button
                                        onClick={() => setActiveTab('registrations')}
                                        className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'registrations' ? 'bg-slate-800 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}
                                    >
                                        Pending
                                    </button>
                                </div>
...
function StoreCard({ store, formatCurrency }: { store: ReferralStoreStats, formatCurrency: any }) {
    ...
    return (
        <div className="bg-slate-900/60 border border-slate-800 rounded-3xl overflow-hidden">
            <div className="p-4 flex items-center gap-4 cursor-pointer hover:bg-slate-800/50 transition-colors" onClick={() => setIsExpanded(!isExpanded)}>
                ...
                <div className="flex-1 min-w-0">
                    <h3 className="font-bold text-white truncate text-sm">{store.name}</h3>
                    <div className="flex items-center gap-2 mt-1">
                        <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full uppercase ${store.status === 'active' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/10' : 'bg-amber-500/20 text-amber-400 border border-amber-500/10'}`}>
                            {store.status}
                        </span>
                ...
                <div className="text-right">
                    {store.commission.weeklyAmount > 0 ? (
                        <>
                            <div className="text-xs font-black text-emerald-400">+{formatCurrency(store.commission.weeklyAmount)}</div>
                            <div className="text-[9px] text-slate-500 font-bold uppercase mt-0.5">Weekly</div>
                        </>
                    ) : (
                        <div className="text-[10px] font-bold text-slate-500 uppercase">No Comm.</div>
                    )}
                </div>
                    ...
                    >
                        {store.status === 'trial' && store.trialEndsAt && (
                            <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-3 flex items-center gap-3">
                                ...
                                <div className="text-[11px] text-amber-100/80">
                                    Trial Ends: <span className="text-amber-400 font-bold">{new Date(store.trialEndsAt).toLocaleDateString()}</span>
                                </div>
                            </div>
                        )}
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div className="bg-slate-900 border border-slate-800 p-3 rounded-2xl">
                                ...
                                <div className="text-lg font-black text-white">{store.weeklyPerformance.views.current.toLocaleString()}</div>
                            </div>
                            <div className="bg-slate-900 border border-slate-800 p-3 rounded-2xl">
                                <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Orders</div>
                                <div className="text-lg font-black text-white">{store.weeklyPerformance.orders.current.toLocaleString()}</div>
                            </div>
                        </div>
...
function RegistrationCard({ reg }: { reg: ReferralRegistration }) {
    return (
        <div className="bg-slate-900/40 border border-slate-800 rounded-3xl p-4 flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
                <h3 className="font-bold text-white truncate text-sm">{reg.businessName}</h3>
            ...
            <div className={`px-3 py-1 rounded-full text-[9px] font-black uppercase border ${reg.status === 'completed' || reg.status === 'active' ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/20' : 'bg-blue-500/20 text-blue-400 border-blue-500/20'}`}>
    ...
    return (
        <div className="absolute inset-0 z-50 flex items-end sm:items-center justify-center">
            ...
            <motion.div
                ...
                animate={{ y: 0 }}
                exit={{ y: '100%' }}
                className="relative w-full sm:max-w-md bg-slate-900 border border-slate-800 sm:rounded-[32px] rounded-t-[32px] overflow-hidden shadow-2xl flex flex-col max-h-[85vh]"
            >
                <div className="p-6 pb-4 flex items-center justify-between flex-shrink-0 border-b border-slate-800/50">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-emerald-500/10 rounded-xl flex items-center justify-center border border-emerald-500/20">
                            {type === 'tier' ? <Trophy className="w-5 h-5 text-emerald-400" /> :
                                type === 'referrals' ? <Users className="w-5 h-5 text-blue-400" /> :
                                    type === 'active' ? <Store className="w-5 h-5 text-emerald-400" /> :
                                        type === 'earnings' ? <CreditCard className="w-5 h-5 text-amber-400" /> :
                    ...
                    <button onClick={onClose} className="w-8 h-8 bg-slate-800 rounded-full flex items-center justify-center text-slate-400 hover:text-white transition-colors">
                        <X className="w-4 h-4" />
                    </button>
                </div>
                    ...
                    {type === 'tier' && (
                        <div className="space-y-3">
                            ...
                            { name: 'Elite', commission: 30, range: '11+ Stores' }].map((t) => {
                                ...
                                return (
                                    <div key={t.name} className={`p-4 rounded-2xl border transition-all ${isCurrent ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-slate-800/30 border-slate-800'}`}>
                                        <div className="flex items-center justify-between mb-2">
                                            <h3 className={`text-sm font-black uppercase tracking-tight ${isCurrent ? 'text-emerald-400' : 'text-slate-400'}`}>{t.name}</h3>
                                            {isCurrent && <span className="text-[9px] font-black bg-emerald-500 text-black px-2 py-0.5 rounded-full uppercase">Current</span>}
                                        </div>
                                        <div className="flex items-end justify-between">
                                            <div>
                                                <div className="text-xl font-black text-white">{t.commission}%</div>
                                            </div>
                                            <div className="text-right">
                                                <div className="text-xs font-bold text-slate-300">{t.range}</div>
                                            </div>
                                        </div>
                                    </div>
                                );
...
function StatBox({ label, value, icon: Icon, color }: any) {
    ...
    return (
        <div className={`p-4 rounded-3xl border ${colors[color]} bg-slate-900/40`}>
