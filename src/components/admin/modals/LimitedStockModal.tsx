...
const LimitedStockModal: React.FC<LimitedStockModalProps> = ({
  limitedStock,
  products,
  setActiveSection,
  setManageTab,
  ...
  const getButtonText = () => {
    ...
    if (!hasLimitedStockItems) return 'Check Stock Levels';
    return 'View All Limited Stock';
  };
  ...
  return (
    <Modal open={true} onClose={handleClose}>
      ...
      <div className="w-full flex flex-col items-center">
        {/* Header */}
        <div className="flex items-center justify-center w-14 h-14 rounded-2xl bg-warning/10 mb-4 border border-warning/20">
          <AlertTriangle className="w-7 h-7 text-warning" />
        </div>
        <h2 className="text-xl font-bold text-text-primary">Limited Stock</h2>
        <p className="text-4xl font-bold text-warning my-1">{limitedStock}</p>
        <p className="text-sm text-text-secondary mb-6 text-center">Products that are running low on stock.</p>
        {/* Content */}
        <div className="w-full">
          {!hasLimitedStockItems ? (
            <div className="w-full mt-2 flex flex-col items-center text-center">
              <p className="text-sm text-text-secondary mb-4">
                {isEmpty ? 'No products have been added yet.' : 'Good job! All products are well-stocked.'}
              </p>
            </div>
          ) : (
        ...
        {/* Footer Button */}
        <button
          className="mt-6 w-full px-4 py-2.5 rounded-lg bg-warning text-warning-foreground font-semibold hover:bg-warning/90 transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-warning focus-visible:ring-offset-2 focus-visible:ring-offset-background"
