...
const RevenueAnalyticsView = ({ data }: { data: RevenueData }) => {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
        <div className="bg-green-50 dark:bg-gradient-to-br dark:from-green-600 dark:via-emerald-700 dark:to-teal-900 p-4 sm:p-6 rounded-2xl shadow-md border border-green-100 dark:border-white/5">
          <p className="font-black text-[10px] uppercase tracking-widest text-green-800 dark:text-green-300">Released Revenue</p>
          <p className="text-3xl sm:text-4xl font-black flex items-center text-slate-900 dark:text-white mt-2">
            <Naira />{(data.lifetimeRevenue + data.lifetimeBonus).toLocaleString()}
          </p>
          {data.lifetimeBonus > 0 && (
            <p className="text-[10px] font-bold text-green-700 dark:text-green-400 mt-1 uppercase tracking-tight">
              Includes <Naira />{data.lifetimeBonus.toLocaleString()} Bonus
            </p>
          )}
        </div>
        <div className="bg-orange-50 dark:bg-gradient-to-br dark:from-orange-600 dark:via-amber-600 dark:to-yellow-700 p-4 sm:p-6 rounded-2xl shadow-md border border-orange-100 dark:border-white/5 text-white">
          <p className="font-black text-[10px] uppercase tracking-widest text-orange-800 dark:text-orange-200">Pending Escrow</p>
          <div className="flex flex-col">
            <p className="text-3xl sm:text-4xl font-black flex items-center text-slate-900 dark:text-white mt-2">
              <Naira />{(data.pendingEscrow || 0).toLocaleString()}
            </p>
            <p className="text-[10px] font-bold text-orange-700 dark:text-orange-300 mt-1 uppercase tracking-tight">
              Awaiting Deliverable Approval
            </p>
          </div>
        </div>
      </div>
        ...
        >
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6">
              <div className="flex items-center justify-between py-4 sm:py-6 border-b border-slate-200 dark:border-slate-800">
                <div>
                  ...
                  <p className="text-[10px] font-bold text-orange-500 uppercase tracking-widest">Revenue & Escrow Analytics</p>
                </div>
          ...
          <main className="flex-grow w-full max-w-5xl mx-auto overflow-y-auto p-4 sm:p-6 scrollbar-hide">
            {isLoading ? (
              ...
              <RevenueAnalyticsView data={data} />
            ) : (
