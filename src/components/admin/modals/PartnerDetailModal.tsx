      ...
      {isOpen && (
        ...
        >
          ...
          >
            ...
            {/* Mobile Tab Bar */}
            <div className="md:hidden flex gap-2 p-4 border-b border-gray-200 bg-white sticky top-16 z-20">
              <button
            ...
            <div className="flex flex-1 overflow-hidden flex-col md:flex-row">
              {/* Products Section */}
              <div className={`flex-1 overflow-y-auto p-4 md:p-6 ${activeTab === 'cart' ? 'hidden md:flex md:flex-col' : 'flex flex-col'}`}>
                {loading ? (
                        ...
                        <div className="p-3 sm:p-4">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className="flex-shrink-0 w-8 h-8 rounded-full bg-gray-200 hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center transition"
                            >
                              <Minus size={16} className="text-gray-700" />
                            </button>

                            {/* Quantity Display */}
                            <span className="flex-shrink-0 w-8 text-center font-semibold text-sm">
                            ...
                            <button
                              ...
                              className="flex-shrink-0 w-8 h-8 rounded-full bg-gray-200 hover:bg-gray-300 flex items-center justify-center transition"
                            >
              ...
              {/* Cart Sidebar / Tab */}
              <div className={`w-full md:w-80 border-t md:border-t-0 md:border-l border-gray-200 bg-gray-50 p-4 md:p-6 overflow-y-auto flex flex-col ${activeTab === 'products' ? 'hidden md:flex' : 'flex'}`}>
                <div className="hidden md:flex items-center gap-2 mb-4 md:mb-6 pb-4 border-b border-gray-200">
                ...
                {cartItems.length === 0 ? (
                  <div className="flex-1 flex flex-col items-center justify-center py-8 md:py-12">
                    ...
                    <p className="text-gray-600 font-medium text-center mb-2">Your cart is empty</p>
                    <p className="text-sm text-gray-500 text-center mb-4">Browse our products to get started</p>
                    {activeTab === 'cart' && (
                      <button
                        onClick={() => setActiveTab('products')}
                  ...
                  <>
                    {/* Cart Items */}
                    <div className="flex-1 space-y-2 mb-4 sm:mb-6 overflow-y-auto pr-2">
                      {cartItems.map((item) => (
                        <div key={item.productId} className="bg-white rounded-lg p-2 sm:p-3 border border-gray-200">
                          <div className="flex justify-between items-start mb-2 gap-2">
                            <p className="font-semibold text-xs sm:text-sm line-clamp-2 flex-1">{item.name}</p>
                            <button
                              onClick={() => handleRemoveFromCart(item.productId)}
                              className="text-red-600 hover:text-red-700 flex-shrink-0 transition"
                            >
                              <Trash2 size={16} />
                            </button>
                          </div>
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex items-center gap-1">
                              <button
                                ...
                                className="flex-shrink-0 w-6 h-6 rounded bg-gray-200 hover:bg-gray-300 flex items-center justify-center transition"
                              >
                                <Minus size={12} />
                              </button>
                              <span className="flex-shrink-0 w-6 text-center font-semibold text-xs">{item.quantity}</span>
                              <button
                                onClick={() => handleUpdateCartQuantity(item.productId, item.quantity + 1)}
                                className="flex-shrink-0 w-6 h-6 rounded bg-gray-200 hover:bg-gray-300 flex items-center justify-center transition"
                              >
                    ...
                    <div className="mb-4 pb-4 border-b border-gray-200">
                      <label className="block text-xs sm:text-sm font-semibold text-gray-900 mb-2">Payment Terms</label>
                    ...
                    {minOrder > 0 && cartTotal < minOrder && (
                      <div className="bg-red-50 border border-red-200 rounded-lg p-2 sm:p-3 mb-3 flex items-start gap-2">
                        <AlertCircle size={14} className="text-red-600 mt-0.5 flex-shrink-0 min-w-fit" />
                    ...
                    {cartItems.length > 0 && canProceed && (
                      <div className="bg-green-50 border border-green-200 rounded-lg p-2 sm:p-3 mb-3 flex items-start gap-2">
