...
const ShareReferralModal = ({ isOpen, onClose, storeName, referralLink }: ShareReferralModalProps) => {
  ...
  const shareActions = [
    { name: 'WhatsApp', icon: <WhatsAppIcon />, color: 'bg-green-500', action: () => {
    ...
    { name: 'Facebook', icon: <FacebookIcon />, color: 'bg-blue-600', action: () => {
       const url = encodeURIComponent(referralLink);
       window.open(`https://www.facebook.com/sharer/sharer.php?u=${url}`);
    }},
    { name: 'Instagram', icon: <InstagramIcon />, color: 'bg-pink-500', action: () => {
      navigator.clipboard.writeText(referralLink);
      toast.success('Link copied! Paste it in your Instagram story or bio.');
    }},
    { name: 'Snapchat', icon: <SnapchatIcon />, color: 'bg-yellow-400', action: () => {
      navigator.clipboard.writeText(referralLink);
              ...
              <Dialog.Panel className="relative w-full max-w-md transform text-left transition bg-slate-100 dark:bg-slate-900 shadow-2xl rounded-t-2xl sm:rounded-2xl">
                <div className="p-4 sm:p-6">
                  <div className="flex justify-between items-center mb-4">
                    <Dialog.Title as="h3" className="text-xl font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                      <Share2 size={22} />
                      Share & Earn
                    </Dialog.Title>
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400 mb-6">
                    Share the {storeName} online store with loved ones and earn referral bonuses, discounts, and gifts when they shop.
                  </p>
                  ...
                  <div className="grid grid-cols-4 gap-4 mb-6">
                    {shareActions.map(action => (
                      <button
                        ...
                        className={`flex flex-col items-center justify-center p-3 rounded-xl text-white aspect-square transition-transform hover:scale-105 ${action.color}`}
                      >
                        {action.icon}
                ...
                <div className="bg-slate-200/50 dark:bg-slate-800/50 p-4 sm:hidden">
