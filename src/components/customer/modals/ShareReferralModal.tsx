...
const ShareReferralModal = ({ isOpen, onClose, storeName, referralLink }: ShareReferralModalProps) => {
  ...
  const shareActions = [
    ...
    {
      name: 'Snapchat', icon: <SnapchatIcon />, color: 'border-yellow-500/30 text-yellow-400 bg-yellow-500/5', action: () => {
        navigator.clipboard.writeText(referralLink);
              ...
              <Dialog.Panel className="relative w-full max-w-md transform text-left transition bg-gradient-to-br from-indigo-950 via-slate-900 to-indigo-950 shadow-2xl rounded-t-[2rem] sm:rounded-[2rem] border-t sm:border border-white/10 overflow-hidden">
                ...
                <div className="relative z-10 p-6 sm:p-8">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <button onClick={onClose} className="p-2 rounded-full bg-white/5 hover:bg-white/10 border border-white/10 transition-colors">
                ...
                <div className="relative z-10 p-6 bg-white/5 border-t border-white/5 sm:hidden">
                  <button onClick={onClose} className="w-full bg-white/10 hover:bg-white/20 text-white font-black py-4 px-4 rounded-2xl border border-white/10 transition-all active:scale-[0.98]">
                    Done
                  </button>
                </div>
              </Dialog.Panel>
