...
const CustomerLookupModal = ({ isOpen, onClose, onSuccess }: CustomerLookupModalProps) => {
    ...
    const handleEmailSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await updateCustomerEmail(foundCustomer.id, email);
            const updatedCustomer = { ...foundCustomer, email };
            ...
            case "CollectEmail":
                return (
                    <form onSubmit={handleEmailSubmit} className="space-y-6">
                        ...
                        <button type="submit" disabled={isSubmittingEmail} className="w-full bg-slate-800 text-white font-bold py-3 rounded-2xl mt-4 hover:bg-slate-900 transition-colors disabled:bg-slate-600 shadow-lg">
                            {isSubmittingEmail ? "Updating..." : "Continue to Order"}
                        </button>
                    </form>
                )
            case "AllDone":
